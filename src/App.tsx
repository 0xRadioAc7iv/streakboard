...
function App() {
  ...
  const fetchTasks = async (initialData: StreakData = streakData) => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from("tasks").select("date, task");
  ...
  const storeTask = async (date: string, task: string) => {
    ...
    try {
      const { error } = await supabase.from("tasks").insert([{ date, task }]);
  ...
  return (
    <div className="flex flex-col min-h-screen p-6 bg-black text-white">
      <h1 className="text-2xl font-bold text-center mb-6">Streakboard</h1>
